...
here is the evidence that these ones are right, measured two independent
ways. First, 1,312 gradient checks — one hand-computed partial derivative
against one central difference, for every scalar of every parameter in a
2-block model. `examples/gradcheck.py` sweeps all 1,312 in about half a
second. CI checks all 29 parameter tensors on every push, sampling
coordinates inside the large ones so that test stays under a second.
Second, against the implementation the rest of the world uses: CI's
`parity` job rebuilds the model with `torch` ops, copies these weights
across, and diffs every parameter gradient — the two agree to about
`1e-15` in float64, on every push, across five architectures
(`tests/test_torch_parity.py`). torch stays out of the package; it is an
`[audit]` extra the job installs to check the NumPy against.

1,312 counts derivatives, not tests. The suite is 194 tests, and the
